...
class _CityDashboardScreenState extends State<CityDashboardScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _buildSeverityChart(bool isDarkMode) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          _buildSeverityBar('Critical', severity['critical'] ?? 0, total, Colors.red, '🔴', isDarkMode),
          const SizedBox(height: 12),
          _buildSeverityBar('High', severity['high'] ?? 0, total, Colors.deepOrange, '🟠', isDarkMode),
          const SizedBox(height: 12),
          _buildSeverityBar('Medium', severity['medium'] ?? 0, total, Colors.orange, '🟡', isDarkMode),
          const SizedBox(height: 12),
          _buildSeverityBar('Low', severity['low'] ?? 0, total, Colors.green, '🟢', isDarkMode),
        ],
      ),
    );
  }
  ...
  Widget _buildBadgesSection(bool isDarkMode) {
    ...
    final allBadges = [
      ...
      {'id': 'reporter_10', 'name': '10 Reports', 'icon': '🥉', 'desc': 'Submit 10 reports'},
      {'id': 'reporter_25', 'name': '25 Reports', 'icon': '🥈', 'desc': 'Submit 25 reports'},
      {'id': 'reporter_50', 'name': '50 Reports', 'icon': '🥇', 'desc': 'Submit 50 reports'},
      {'id': 'reporter_100', 'name': '100 Reports', 'icon': '🏆', 'desc': 'Submit 100 reports'},
    ];
  ...
  void _shareImpact() {
    ...
    final text = '''
🦸 My Pothole Hero Impact 🦸

📊 My Stats:
• Reports Submitted: $reports
• Issues Resolved: $resolved
• Roads Improved: ${roads.toStringAsFixed(0)}m

I'm helping make Bengaluru's roads safer! 🛣️
